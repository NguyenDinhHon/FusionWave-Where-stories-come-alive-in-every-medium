...
/// Premium Library Page với design giống Wattpad & Waka
class PremiumLibraryPage extends ConsumerStatefulWidget {
  const PremiumLibraryPage({super.key});

  @override
  ConsumerState<PremiumLibraryPage> createState() => _PremiumLibraryPageState();
}

class _PremiumLibraryPageState extends ConsumerState<PremiumLibraryPage>
            ...
            child: Column(
              children: [
                ...
                TabBar(
                  ...
                  unselectedLabelColor: AppColors.textPrimaryLight.withOpacity(0.7),
  ...
  List<LibraryItemModel> _applyFiltersAndSort(
    ...
    if (_selectedAuthor != null && _selectedAuthor!.isNotEmpty) {
      filtered = filtered.where((item) {
        final book = bookMap[item.bookId];
        return book != null && book.authors.any((author) =>
  ...
  Widget _buildLibraryList(String? status) {
    ...
    return libraryItemsAsync.when(
      ...
      loading: () => _isGridView
          ...
          : ListView.builder(
              padding: EdgeInsets.all(AppBreakpoints.padding(MediaQuery.of(context).size.width)),
              itemCount: 5,
  ...
  Widget _buildLibraryItemGridCard(BuildContext context, LibraryItemModel item) {
    final bookAsync = ref.watch(bookByIdProvider(item.bookId));
